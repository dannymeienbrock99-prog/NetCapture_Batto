...
2. `CrazyBatto-NetCapture-Setup-v0.6.6.zip` vollständig entpacken und anschließend `CrazyBatto-NetCapture-Setup-v0.6.6.exe` im entpackten Ordner starten. Die Windows-Abfrage für Administratorrechte bestätigen.
...
6. NetCapture erstellt oder aktualisiert automatisch eine SRT-Medienquelle in dieser Szene.
7. Für Ton einmal **Tonquellen laden** drücken und den gewünschten `PC-Ton:`-Ausgang oder ein `Mikrofon:` auswählen.
8. Danach **Übertragung starten** drücken.
...
Mit Version 0.6.6 ist für einen Funktionstest kein zweiter Rechner nötig:
...
3. Rechts oben in NetCapture **Dieser PC testen** aktivieren. NetCapture trägt für SRT und OBS WebSocket automatisch `127.0.0.1` ein.
4. Das OBS-WebSocket-Passwort eingeben und **Lokal mit OBS verbinden** drücken.
5. Eine Szene wählen, **Quelle einrichten** drücken und danach die Übertragung starten.

Im lokalen Test ist keine Windows-Firewall-Freigabe erforderlich. NetCapture merkt sich die zuvor eingetragene Adresse des anderen OBS-PCs und stellt sie wieder her, sobald **Dieser PC testen** deaktiviert wird. Bei einer vollständigen Bildschirmaufnahme sollte OBS minimiert oder eine Fenster-/Spielaufnahme gewählt werden, damit kein Endlos-Spiegeleffekt entsteht.
...
Ein einzelner H.264-Stream kann diese Breite nicht zuverlässig codieren. Version 0.6.6 startet deshalb im Modus **UltraWide Triple-Split** drei getrennte FFmpeg-Prozesse:
...
- **Verbindung wird nach dem Laden der Szenen wieder getrennt / Eigenschaft `scenes` fehlt:** Version 0.6.6 installieren. Die OBS-Antwort wird dort ohne PowerShell-Aufzählung verarbeitet; notfalls verwendet NetCapture die aktuell aktive OBS-Szene und hält die Verbindung offen.
- **Ungültiger URI / ungültiger Anschluss:** Version 0.6.6 akzeptiert im Hostfeld sowohl `127.0.0.1` als auch `127.0.0.1:4455` und `ws://127.0.0.1:4455`. Im lokalen Test wird automatisch die richtige Adresse gesetzt.
...
- **Alle drei Triple-Streams melden gleichzeitig Code -5:** Die Bildschirmaufnahme ist in diesem Fall in Ordnung, aber OBS lauscht noch nicht auf `9000–9002`. Zuerst NetCapture per WebSocket mit OBS verbinden und **Quelle einrichten** drücken. Version 0.6.6 startet die Empfänger danach vor jeder Übertragung automatisch neu und lässt den SRT-Callern 20 Sekunden Verbindungszeit.
...
- **AudioPipeCapture.dll fehlt:** Version 0.6.6 mit dem fertigen Setup erneut installieren. Die Quelldatei `AudioPipeCapture.cs` darf nicht mehr beim Programmstart kompiliert werden.
- **Fehler 4551 / Datei konnte nicht im temporären Ordner ausgeführt werden:** Nur das v0.6.6-ZIP verwenden, vollständig entpacken und alle `.exe`-/`.bin`-Teile im selben Ordner lassen. v0.6.6 verwendet keinen temporär gestarteten Inno-Setup-Loader mehr.
- **Nach dem Start öffnet sich kein Fenster:** Prüfe `%LOCALAPPDATA%\CrazyBatto\NetCapture\launcher.log`. Version 0.6.6 zeigt bei einem Launcher- oder PowerShell-Abbruch zusätzlich ein sichtbares Fehlerfenster an.
